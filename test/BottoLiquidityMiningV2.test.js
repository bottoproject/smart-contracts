...
contract("BottoLiquidityMiningV2", (accounts, network) => {
    ...
    context("deposit reward tokens", function() {
        ...
        context("advance to start time", function() {
            ...
            context("stakers stakes", function() {
                beforeEach(async function() {
                ...
                context("advance to half stake time", function() {
                    ...
                    context("upgrade contract", function() {
                        ...
                        context("extend stake time", function() {
                            beforeEach(async function() {
                                newEndTime = (await miningProxy.endTime()).add(time.duration.years(2));
                            ...
                            it("has expected end time", async function() {
                                expect(await miningProxy.endTime.call()).to.be.bignumber.equal(newEndTime);
                            });

                            it("has same rewards at the time of the extend", async function() {
                                newReward1 = await miningProxy.payout.call({ from: staker1 });
                                expect(newReward1).to.be.bignumber.closeTo(reward1, reward1.div(toBN("100000")));
                            });

                            it("has same total deposit rewards as the initial", async function() {
                            ...
                            context("advance to half new stake time", async function() {
                                beforeEach(async function() {
                                    previousRewards = await miningProxy.payout.call({ from: staker1 });
                                    let firstStakeTime = await miningProxy.firstStakeTime();
                                ...
                                it("has expected rewards", async function() {
                                    let currentRewards = await miningProxy.payout.call({ from: staker1 });
                                    expectedRewards = newStakingDuration
                                        .div(toBN("2"))
                                        .mul(rewardPerSecond)
                                        .mul(staker1Initial)
                                    ...
                                    expect(currentRewards).to.be.bignumber.closeTo(
                                        expectedRewards, expectedRewards.div(toBN("100000"))
                                    );
                                });

                                context("reduce stake time", function() {
                                    beforeEach(async function() {
                                        previousRewards = await miningProxy.payout.call({ from: staker1 });
                                        let newEndTime = (await miningProxy.endTime()).sub(time.duration.years(0.5));

                                        await miningProxy.updateEndTime(newEndTime);

                                        let totalRewards = await miningProxy.totalRewards();

                                        firstStakeTime = await miningProxy.firstStakeTime();
                                        newStakingDuration = (await miningProxy.endTime()).sub(firstStakeTime);
                                        rewardPerSecond = totalRewards.div(newStakingDuration);
                            ...
                            context("advance to staking end time", async function() {
                                beforeEach(async function() {
                                    let endTime = await miningProxy.endTime();
